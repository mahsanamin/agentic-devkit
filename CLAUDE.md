...
> **Work directly on `main` in this checkout - no feature branches, no worktree for this repo.** This is an adhoc, personal repo kept deliberately low-friction: edit, commit, and push straight to `main`. There is no `develop` and no feature-branch flow; everything lands on `main`. This is intentional for a concrete reason: the live skills/agents are symlinks into THIS checkout, so a change made in a separate worktree does not go live until it is merged back and this checkout is updated. Editing here makes it live immediately. If a stray feature branch ever shows up, fold its wanted work into `main` and delete it.

> **Whether `main` is the last stop depends on what `origin` is - check `git remote -v` first.** With push access to the canonical repo, `origin` IS that repo, pushing to `main` is the whole story, and there is no PR step. On a machine that works through a **fork** (no push access upstream), `origin` is the fork and a second remote, `upstream`, is the canonical repo. Everything above still holds - commit on `main`, push to `origin/main`, because that is the checkout the live symlinks resolve to - but upstream receives the work as a **pull request from the fork's `main`**. While that PR is open it is the only one: follow-up work is more commits pushed to the same `main`, and the open PR picks them up automatically. Do not open a second PR for the next change, and do not spin up a feature branch to tidy it - that would take the work off the branch the symlinks point at.
